...
export default async function AdminOverviewPage(props: { searchParams?: Promise<{ sort?: string }> }) {
  try {
    ...
    // 1. Fetch total counts with fallbacks
    let productCount = 0;
    let transactionCount = 0;
    try {
      [productCount, transactionCount] = await Promise.all([
    ...
    let salesItems: any[] = [];
    try {
      salesItems = await prisma.transactionItem.findMany({
        where: {
          transaction: {
            status: "settlement"
          }
      ...
      console.error("Admin Sales Fetch Error:", e);
    }
    ...
    try {
      ...
      console.error("Admin Visits Fetch Error:", e);
    }
    ...
    console.error("Critical Admin Overview Error:", err);
    return (
      <div className="p-12 text-center bg-white rounded-3xl border border-stone-200">
        ...
        <p className="text-stone-500">We couldn't load the admin overview. Please try refreshing the page.</p>
      </div>
    );
  }
}
